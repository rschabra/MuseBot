...
async def q(ctx):
    ...
    if len(query) < 5:
        ...
        length_query = 5
    embedVarr = discord.Embed(title="Queue", description="Either the next 5 songs or the rest of the queue.", color=0x00ff00)
    for i in range(0,length_query):
        embedVarr.add_field(name=f"Song {i}", value=query[i])  
